(function($) {
  ...
  function MultipleSelect($el, options) {
    ...
    this.$choice = $('<div class="ms-choice"><span class="placeholder">' + options.placeholder + '</span><div></div></div>');
    ...
    optionToHtml: function(i, elm, group, groupDisabled) {
      ...
      if ($elm.is('option')) {
        ...
        html.push(
          ...
          '<input type="checkbox" name="selectItem" value="' + value + '"' + (selected ? ' checked="checked"' : '') + (disabled ? ' disabled="disabled"' : '') + (group ? ' data-group="' + group + '"' : '') + '/> ',
          text,
          '</label>',
          '</li>'
        );
...
jQuery('document').ready(function() {
  jQuery('select', 'form.searchresultsoptimizer.search-form').multipleSelect();
  
  jQuery('form.searchresultsoptimizer').submit(function(event) {
    jQuery("input[type='hidden']").each(function() {
      var hidden = jQuery(this);
      jQuery(hidden).val('');
      var name = jQuery(hidden).attr('name');
      jQuery('select#' + name).next().find(':checked').each(function() {
        var result = jQuery(this);
        jQuery(hidden).val(function(i,val) {
          return val + (!val ? '' : '|') + jQuery(result).val();
        });
      });
    });    
  });
